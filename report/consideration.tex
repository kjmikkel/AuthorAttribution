...
What the corpus is going to contain - how many entries, the number of posts in it, etc. \ldots
}
...
\subsection{Tests}
I will in this section describe the various texts that I will run. It should be noted that there are no StressTest for \postAmount{few}, since no author has written \postAmount{few} posts. 
...
\test{StressTest2}{
Check a single post from an author who has written \postAmount{Some} posts against all texts written by authors who have written \postAmount{Some} posts
}{
...
\test{StressTest3}{
Check a single post from an author who has written \postAmount{Many} posts against all texts written by authors who have written \postAmount{Many} posts
}{
...
\test{ShortBogusTest}{
Take a post from an author A with multiple \postSize{Long} posts, and create a corpora from all the texts of A, and a short bogus post from a bogus author.
}{
This test to check whether the algorithm gives a higher probability to authors with very little information - even if the bogus post is very dissimilar to the post that it is being checked against.
}{
1 long post from A 
}{
...
A simple test to see whether the algorithm will work better if all authors have written a comparatively equal amount of texts, and there are \postAmount{Some} of them.
}{
...
A simple test to see whether the algorithm will work better if all authors have written a comparatively equal amount of texts, and there are \postAmount{Many} of them.
}{
...
Take a random selection of authors, who have made over half the posts in the entire corpora, and test it on the rest of the corpora. 
}{
This test is to check the overall quality of the algorithm - i.e. if it can attribute the posts correctly, based on the five attributes in the test section, and compared to the results found in \cite{nr4}. Since the authors are chosen at random, some of the runs may have many authors (with a low average of posts), while others will only have few (but with a larger average of posts).
}{
